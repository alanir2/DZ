...
#     all_marcet_cap = soup.find_all(class_="sc-11478e5d-1 hwOFkt")
#
#     total_value = sum([int(value.text.replace('$', '').replace(',', '')) for value in all_marcet_cap])
#     current_time = datetime.now().strftime('%H.%M %d.%m.%Y')
#
#     for name, valuse in zip(all_coins, all_marcet_cap):
#         num = int(valuse.text.replace('$', '').replace(',', ''))
...
#         for name, valuse in zip(all_coins, all_marcet_cap):
#             name_coin = name.text
#             market_cap = valuse.text
#             num = int(valuse.text.replace('$', '').replace(',', ''))
#             num = float(num / total_value * 100)
#             percent = str(round(num, 2)) + '%'
#             print(name_coin, market_cap, percent)
#
# # добовляем данные в уже созданный файл
#
#             with open(f'data2/{current_time}.csv', 'a', encoding='utf8') as file:
